...
export function AdminLayout({ children }: AdminLayoutProps) {
  ...
  const navigation = [
    ...
    { name: 'FAQs', href: '/faqs', icon: 'M8.228 9c.549-1.165 2.03-2 3.772-2 2.21 0 4 1.343 4 3 0 1.4-1.278 2.575-3.006 2.907-.542.104-.994.54-.994 1.093m0 3h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z' },
    ...
    { name: 'Inquiries', href: '/inquiries', icon: 'M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-3 7h3m-3 4h3m-6-4h.01M9 16h.01' },
    { name: 'Unanswered', href: '/unanswered', icon: 'M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z' },
    ...
    ...(isSuperAdmin ? [
      { name: 'Users', href: '/users', icon: 'M12 4.354a4 4 0 110 7.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197m13.5-9a2.5 2.5 0 11-5 0 2.5 2.5 0 015 0z' },
      ...
      { name: 'System Logs', href: '/system-logs', icon: 'M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z' },
    ] : []),
    { name: 'Translations', href: '/translations', icon: 'M3 5h12M9 3v2m1.048 9.5A18.022 18.022 0 016.412 9m6.088 9h7M11 21l5-10 5 10M12.751 5C11.783 10.77 8.07 15.61 3 18.129' },
    ...
    <div className="min-h-screen bg-gray-100 flex">
      ...
      <div className={`${sidebarOpen ? 'w-64' : 'w-20'} bg-gray-900 transition-all duration-300 flex flex-col`}>
        <div className="flex items-center justify-between h-16 px-4">
          {sidebarOpen && (
            <span className="text-xl font-bold text-white">NestChat</span>
          )}
          <button
        ...
        <nav className="flex-1 px-2 py-4 space-y-1">
          {navigation.map((item) => {
            ...
            return (
              ...
              >
                <svg className={`h-5 w-5 ${sidebarOpen ? 'mr-3' : 'mx-auto'}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <div className="p-4 border-t border-gray-700">
          {sidebarOpen ? (
            ...
            <div className="w-8 h-8 bg-blue-600 rounded-full flex items-center justify-center text-white font-medium mx-auto">
              {user?.name?.charAt(0) || 'U'}
            </div>
          )}
        </div>
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <header className="bg-white shadow-sm h-16 flex items-center justify-between px-6">
          ...
          <div className="flex items-center gap-4">
            ...
            <span className={`w-2 h-2 rounded-full ${connected ? 'bg-green-500' : 'bg-red-400'}`} title={connected ? 'Live' : 'Offline'}></span>
